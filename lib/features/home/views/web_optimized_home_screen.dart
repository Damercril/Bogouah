...
/// Version optimisée de l'écran d'accueil pour le web
class WebOptimizedHomeScreen extends StatefulWidget {
  const WebOptimizedHomeScreen({Key? key}) : super(key: key);

  @override
  State<WebOptimizedHomeScreen> createState() => _WebOptimizedHomeScreenState();
}

class _WebOptimizedHomeScreenState extends State<WebOptimizedHomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Sur mobile, afficher la version standard
    if (ResponsiveHelper.isMobile(context)) {
      return _buildMobileLayout(user, isDarkMode);
    }
    
    // Sur web, afficher la version optimisée
    return _buildWebLayout(user, isDarkMode);
  }

  Widget _buildMobileLayout(dynamic user, bool isDarkMode) {
    return Column(
      children: [
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildActivitiesSection(isDarkMode),
  ...
  Widget _buildWebLayout(dynamic user, bool isDarkMode) {
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // Statistiques en grille responsive (4 colonnes sur desktop, 2 sur tablette)
            ResponsiveGrid(
              mobileColumns: 2,
              tabletColumns: 2,
              desktopColumns: 4,
              childAspectRatio: 1.3,
              children: _statsData.map((stat) => _buildStatCard(stat, isDarkMode)).toList(),
            ).animate().fadeIn(duration: 600.ms, delay: 100.ms).slideY(begin: 0.2, end: 0),
            
            const SizedBox(height: 32),
            
            // Layout deux colonnes pour le graphique et les activités
            ResponsiveTwoColumns(
              leftFlex: 2,
              rightFlex: 1,
              spacing: 24,
              left: _buildChartSection(isDarkMode),
              right: _buildActivitiesSection(isDarkMode),
            ),
  ...
  Widget _buildActivitiesSection(bool isDarkMode) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
